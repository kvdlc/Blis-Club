  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-6 text-center relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <h1 className="text-5xl font-extrabold text-primary-700 dark:text-primary-300 mb-3 tracking-tight">
          Blis Club
        </h1>
        <p className="text-lg text-zinc-500 dark:text-zinc-400 max-w-md mb-10 leading-relaxed">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Link
            href="/guau/app"
            className="rounded-[1.25rem] border border-primary-200 text-primary-700 dark:text-primary-300 px-8 py-3.5 font-bold transition-all hover:bg-primary-50 dark:hover:bg-primary-950/40"
          >
            Ya tengo cuenta
          </Link>
        </div>
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl">
          {[
            { title: "Nutrición BARF", desc: "Calculadora inteligente y 150 recetas naturales", color: "bg-secondary-50 dark:bg-secondary-950/40 border-secondary-100 dark:border-secondary-900" },
            { title: "Academia Canina", desc: "Entrenamiento paso a paso estilo Duolingo", color: "bg-warning-50 dark:bg-warning-950/40 border-warning-100 dark:border-warning-900" },
            { title: "Tracker de Paseos", desc: "Registra, mide y mejora cada salida", color: "bg-accent-50 dark:bg-accent-950/40 border-accent-100 dark:border-accent-900" },
          ].map((f) => (
            <div key={f.title} className={`${f.color} rounded-[1.5rem] p-6 border shadow-sm`}>
              <h3 className="font-bold text-zinc-800 dark:text-zinc-200 mb-1.5">{f.title}</h3>
