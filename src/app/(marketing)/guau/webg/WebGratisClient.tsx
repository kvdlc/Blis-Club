...
/* ─── Data ─── */
const PAIN_POINTS = [
  { icon: <Scale className="w-6 h-6" />, title: "Sobrepeso canino", desc: "El 56% de los perros en Latinoamérica tienen sobrepeso por croquetas procesadas." },
  { icon: <Pill className="w-6 h-6" />, title: "Enfermedades evitables", desc: "Diabetes, alergias y problemas renales causados por mala alimentación." },
  { icon: <Activity className="w-6 h-6" />, title: "Falta de ejercicio", desc: "Paseos sin control que no ejercitan correctamente a tu perro." },
  { icon: <Stethoscope className="w-6 h-6" />, title: "Vet caro e impredecible", desc: "Visitas de emergencia que podrían evitarse con prevención." },
];

const SOLUTION_PILLARS = [
  { icon: <Bone className="w-8 h-8" />, title: "Nutrición Mixta Inteligente", desc: "150+ recetas personalizadas. Calculadora automática que ajusta la ración según peso, edad y actividad.", color: "from-secondary-400 to-secondary-600" },
  { icon: <GraduationCap className="w-8 h-8" />, title: "Academia Canina Pro", desc: "Entrenamiento gamificado estilo Duolingo. Obediencia, agilidad, seguridad y socialización paso a paso.", color: "from-primary-400 to-primary-600" },
  { icon: <Heart className="w-8 h-8" />, title: "Tracker de Salud 360°", desc: "Vacunas, peso, visitas, medicamentos y veterinarios. Todo el historial médico en un solo lugar.", color: "from-accent-400 to-accent-600" },
];
...
const BEFORE_AFTER = [
  { label: "Antes", title: "Alimentación desordenada", desc: "Croquetas procesadas, sobrealimentación por ansiedad, golosinas sin control y un perro con sobrepeso, mal aliento y heces enormes.", bad: true, icon: <Pill className="w-5 h-5" /> },
  { label: "Después", title: "Dieta Mixta estructurada", desc: "Recetas balanceadas con gramos exactos, menús semanales claros y un perro con energía estable, pelaje brillante y digestión perfecta.", bad: false, icon: <Bone className="w-5 h-5" /> },
  { label: "Antes", title: "Entrenamiento caótico", desc: "Videos contradictorios, métodos que no se adaptan a tu raza, frustración diaria y un perro que te ignora hasta con premio en la mano.", bad: true, icon: <Activity className="w-5 h-5" /> },
  { label: "Después", title: "Academia gamificada", desc: "Lecciones diarias progresivas, seguimiento visual de avance, logros desbloqueables y un perro que obedece en casa y en la calle.", bad: false, icon: <GraduationCap className="w-5 h-5" /> },
  { label: "Antes", title: "Salud reactiva", desc: "Vet solo en emergencias costosas, vacunas olvidadas, historial disperso en papeles y un perro expuesto a enfermedades prevenibles.", bad: true, icon: <Stethoscope className="w-5 h-5" /> },
  { label: "Después", title: "Salud preventiva", desc: "Recordatorios automáticos, historial completo siempre en el celular, alertas inteligentes y un perro protegido antes de que algo pase.", bad: false, icon: <Heart className="w-5 h-5" /> },
];

const FAQS = [
  { q: "¿Cómo empiezo a usar Blis?", a: "Solo necesitas crear una cuenta con tu correo y contraseña. Una vez dentro, registras a tu perro y listo: tendrás acceso a todas las herramientas." },
  ...
  { q: "¿Es una aplicación que tengo que instalar?", a: "No. Blis es una aplicación web. La abres desde cualquier navegador en tu computadora, tablet o celular. Siempre tendrás tus datos sincronizados." },
];

/* ─── Component ─── */
export function WebGratisClient({ plans }: Props) {
  ...
  return (
    <div className="relative overflow-hidden bg-primary-50" style={{ minHeight: "100dvh" }}>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/90 dark:bg-zinc-900/90 backdrop-blur-md shadow-sm border-b border-zinc-100 dark:border-zinc-800" : "bg-transparent"}`}>
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6 text-xs font-bold text-zinc-500 dark:text-zinc-400">
            <a href="#registro" className="hover:text-primary-500 transition-colors">Registro</a>
          ...
          <a href="#registro" className="text-xs font-bold bg-gradient-to-r from-primary-600 to-primary-500 text-white px-4 py-2 rounded-xl shadow-lg shadow-primary-500/25 hover:shadow-primary-500/40 transition-all active:scale-[0.97]">
            Empezar GRATIS
          </a>
        </div>
      </nav>
      ...
      <section className="relative z-10 px-4 pb-12 max-w-5xl mx-auto">
        <div className="max-w-md mx-auto bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-lg border border-primary-100 dark:border-primary-800/30">
          <h3 className="text-center text-lg font-extrabold text-zinc-900 dark:text-white mb-4">
            Comienza ahora
          </h3>
          <InlineRegister
            appSlug="guau"
      ...
      {/* ═══ REGISTRO GRATIS #1 ═══ */}
      <section id="registro" className="relative z-10 px-4 py-16 max-w-5xl mx-auto">
        <div className="text-center mb-8">
          <span className="text-xs font-bold text-primary-600 dark:text-primary-400 uppercase tracking-wider">Sin compromiso</span>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-zinc-900 dark:text-white mt-2">
            Empieza sin compromiso
          </h2>
          <p className="text-sm text-zinc-500 mt-2 max-w-lg mx-auto">
            Solo necesitas tu correo y una contraseña para comenzar a transformar la vida de tu perro.
          </p>
        </div>
        <div className="max-w-md mx-auto">
          <InlineRegister
            ...
            placeholder="tu@correo.com"
            className="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-lg border border-primary-100 dark:border-primary-800/30"
          />
        </div>
      </section>

      {/* ═══ PRICING ═══ */}
      <section id="pricing" className="relative z-10 px-4 py-16 max-w-5xl mx-auto" style={{ display: "none" }} />
          ...
          <div className="relative rounded-[2rem] p-8 text-white border border-primary-200/50 dark:border-primary-800/30 overflow-hidden" style={{ background: "linear-gradient(135deg, #6b63f3 0%, #8b5cf6 50%, #a855f7 100%)" }}>
            ...
            <div className="relative z-10 space-y-4">
              ...
              <p className="text-sm text-white/80">
                Miles de perros ya viven mejor gracias a Blis. El tuyo puede ser el siguiente. Empieza ahora.
              </p>
              ...
              <p className="text-[10px] text-white/60">Sin instalar nada. Abre desde cualquier navegador.</p>
            </div>
          </div>
        </div>
      </section>
