    ...
    <AdminGuard>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold text-zinc-900">Usuarios</h1>
            <p className="text-sm text-zinc-500 mt-1">Gestiona roles, suscripciones y aplicaciones</p>
          </div>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-100">
                    ...
                    <th className="text-left px-4 py-3 font-semibold text-zinc-600">Registro</th>
                    ...
                    <tr key={u.id} className="border-b border-zinc-50 hover:bg-zinc-50/50">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="font-semibold text-zinc-800 block">{u.display_name || "—"}</span>
                            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${ u.role === "superadmin" ? "bg-accent-100 text-accent-700" : u.role === "admin" ? "bg-primary-100 text-primary-700" : u.role === "institucion" ? "bg-secondary-100 text-secondary-700" : u.role === "suspended" ? "bg-red-100 text-red-700" : "bg-zinc-100 text-zinc-600" }`}>
                              {u.role}
                            </span>
                          </div>
                        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6 space-y-6">
                {/* Header */}
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-bold text-zinc-900">Gestionar usuario</h2>
                ...
                <div className="flex items-center gap-3 p-4 bg-zinc-50 rounded-xl">
                  <div className="w-10 h-10 rounded-full bg-zinc-200 flex items-center justify-center text-zinc-500 text-sm font-bold">
                    {(manageUser.display_name || manageUser.email || "?").charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-zinc-900 truncate">{manageUser.display_name || "—"}</p>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <select
                      ...
                      className="flex-1 rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20"
                    >
                      {roles.map((r) => (
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="p-4 bg-zinc-50 rounded-xl space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${ subscription.status === "active" && !getDaysRemaining( subscription.plan_type === "temporal" ? subscription.expires_at : subscription.current_period_end ).expired ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700" }`}>
                        ...
                        <div className="flex gap-2">
                          <div className="relative flex-1">
                            ...
                            <input
                              ...
                              className="w-full rounded-xl border border-zinc-200 bg-white pl-10 pr-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20"
                            />
                          </div>
                          <button
                            onClick={saveDate}
                    ...
                    {/* Cambiar plan */}
                    <div className="space-y-2">
                      <label className="block text-sm font-medium text-zinc-700">Cambiar plan</label>
                      <div className="grid grid-cols-3 gap-2">
                        {(["temporal", "premium", "permanente"] as const).map((plan) => (
                          <button
                            ...
                            className={`px-3 py-2.5 rounded-xl text-xs font-semibold border transition-colors ${ subscription.plan_type === plan ? "bg-zinc-100 text-zinc-400 border-zinc-200 cursor-not-allowed" : plan === "premium" ? "border-emerald-200 text-emerald-700 hover:bg-emerald-50" : plan === "temporal" ? "border-amber-200 text-amber-700 hover:bg-amber-50" : "border-blue-200 text-blue-700 hover:bg-blue-50" }`}
                          >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <button
                        onClick={askSuspend}
                        disabled={actionLoading}
                        className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold border transition-colors ${ manageUser.role === "suspended" ? "bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100" : "bg-zinc-100 border-zinc-200 text-zinc-700 hover:bg-zinc-200" } disabled:opacity-40`}
                      >
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {(["temporal", "premium", "permanente"] as const).map((plan) => (
                        <button
                          ...
                          onClick={() => updatePlan({ plan_type: plan, status: "active" })}
                          disabled={actionLoading}
                          className={`px-3 py-2 rounded-xl text-xs font-semibold border transition-colors ${ plan === "premium" ? "border-emerald-200 text-emerald-700 hover:bg-emerald-50" : plan === "temporal" ? "border-amber-200 text-amber-700 hover:bg-amber-50" : "border-blue-200 text-blue-700 hover:bg-blue-50" }`}
                        >
                          {plan === "premium" ? "Premium" : plan === "temporal" ? "Temporal" : "Permanente"}
                        </button>
                      ))}
        ...
        {confirmAction && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 space-y-4">
              ...
              <p className="text-sm text-zinc-600 whitespace-pre-line">{confirmAction.message}</p>
