...
export default function PlanesPage() {
  ...
  const [form, setForm] = useState({ name: "", price_cents: 0, max_dogs: 1, features: "" });
  ...
  const handleSave = async () => {
    ...
    setEditing(null);
    setShowNew(false);
    setForm({ name: "", price_cents: 0, max_dogs: 1, features: "" });
    load();
  };
    ...
    <AdminGuard>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => { setEditing(null); setShowNew(true); setForm({ name: "", price_cents: 0, max_dogs: 1, features: "" }); }}
            ...
            {plans.map((p) => (
              <div key={p.id} className="card-soft rounded-[1.25rem] p-5 flex items-center gap-4">
                ...
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-secondary-100 text-secondary-700">
                  {formatPrice(p.price_cents)}/mes
                </span>
                <div className="flex gap-1">
                  <button onClick={() => { setEditing(p); setForm({ name: p.name, price_cents: p.price_cents, max_dogs: p.max_dogs, features: (p.features || []).join(", ") }); }}
