    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-auto-100 flex items-center justify-center">
            <ShoppingBag className="w-5 h-5 text-auto-600" />
          </div>
          <div>
      ...
      <div className="flex gap-2">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-zinc-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-auto-200"
          />
        </div>
        <button onClick={handleSearch}
          className="px-4 py-2.5 rounded-xl bg-zinc-100 text-zinc-600 text-sm font-bold hover:bg-zinc-200 transition-colors">
          Buscar
        </button>
      </div>
      ...
      {listings.length === 0 ? (
        <div className="card-soft rounded-2xl p-8 text-center">
          ...
          <p className="text-sm text-zinc-400">No hay publicaciones {activeCat !== "todas" ? "en esta categoría" : "aún"}</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          {listings.map((listing) => (
            <Link
              ...
              className="card-soft rounded-2xl overflow-hidden hover:shadow-md transition-shadow group"
            >
              {/* Imagen */}
              ...
              <div className="p-3 space-y-1.5">
                <p className="text-xs font-bold text-zinc-800 line-clamp-2 leading-tight group-hover:text-auto-600 transition-colors">
                  {listing.titulo}
                </p>

                <p className="text-sm font-black text-auto-600">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {listing.marca && (
                    <span className="text-[9px] text-zinc-400 truncate">{listing.marca}</span>
                  )}
                </div>
