...
import { Fuel, Gauge, DollarSign, Mountain, Droplets, Scale, ClipboardCheck, Wrench } from "lucide-react";

const tools = [
  { slug: "costo-viaje", icon: Fuel, label: "Costo de viaje", desc: "Estima combustible + peajes para una ruta" },
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-auto-100 flex items-center justify-center">
          <Wrench className="w-5 h-5 text-auto-600" />
        </div>
        <div>
          <h1 className="text-xl font-extrabold text-zinc-800">Herramientas</h1>
      ...
      <div className="grid gap-2">
        {tools.map((tool) => (
          <Link
            ...
            className="card-soft rounded-2xl p-4 flex items-center gap-3 hover:shadow-md transition-shadow active:scale-[0.98]"
          >
            <div className="w-10 h-10 rounded-xl bg-auto-100 flex items-center justify-center shrink-0">
              <tool.icon className="w-5 h-5 text-auto-600" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-bold text-zinc-800">{tool.label}</p>
              <p className="text-xs text-zinc-400 truncate">{tool.desc}</p>
            </div>
          </Link>
        ))}
      </div>
