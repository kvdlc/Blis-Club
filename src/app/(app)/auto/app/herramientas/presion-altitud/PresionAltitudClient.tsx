...
export default function PresionAltitudClient({ defaults }: { defaults: Defaults }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <h1 className="text-xl font-extrabold text-zinc-800">Presión por Altitud</h1>
        <p className="text-xs text-zinc-500 mt-1">Ajusta la presión de los neumáticos al viajar entre la sierra y la costa.</p>
      </div>
      ...
      <div className="card-soft rounded-2xl p-4 space-y-3">
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-7 h-7 rounded-lg bg-auto-100 flex items-center justify-center">
              <Gauge className="w-3.5 h-3.5 text-auto-600" />
            </div>
            <h3 className="text-xs font-bold text-zinc-700">Presión en frío recomendada (PSI)</h3>
          </div>

          <div className="grid grid-cols-2 gap-2">
            <label className="block">
              <span className="text-[10px] font-bold text-zinc-500">Delante</span>
              <div className="flex gap-1 mt-0.5">
                ...
                {defaults.presionDelante && (
                  <button onClick={() => setPresionDelante(defaults.presionDelante!.toString())}
                    className="px-2 py-2 rounded-lg bg-zinc-100 text-[10px] font-bold text-zinc-500 hover:bg-auto-100 hover:text-auto-600">
                    {defaults.presionDelante}
                  </button>
                )}
              </div>
            ...
            <label className="block">
              ...
              <div className="flex gap-1 mt-0.5">
                {usarMismaPresion ? (
                  <div className="flex-1 px-2.5 py-2 rounded-lg bg-zinc-50 text-sm font-medium text-zinc-400 flex items-center">
                    Igual que delante
                  </div>
                ) : (
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-xs font-bold text-zinc-700">Altitudes del viaje</h3>
          </div>

          <div className="grid grid-cols-2 gap-2">
            <label className="block">
              <span className="text-[10px] font-bold text-zinc-500">Altitud de origen (msnm)</span>
        ...
        {/* Referencias rápidas */}
        <div>
          <span className="text-[10px] font-bold text-zinc-500 flex items-center gap-1 mb-2">
          ...
          <div className="flex flex-wrap gap-1">
            {ciudadesPeru.map((c) => (
              <button
                ...
                className="text-[9px] font-medium px-2 py-1 rounded-full bg-zinc-100 text-zinc-600 hover:bg-auto-100 hover:text-auto-600 transition-colors"
              >
      ...
      {resultados ? (
        <div className="space-y-3">
          ...
          <div className={`rounded-2xl p-4 text-center border-2 ${
            resultados.excesivo ? "bg-red-50 border-red-200" : "bg-blue-50 border-blue-200"
          }`}>
            ...
            <p className={`text-lg font-black ${
              resultados.excesivo ? "text-red-700" : "text-zinc-800"
            }`}>
          ...
          <div className="card-soft rounded-2xl p-4">
            <h3 className="text-xs font-extrabold text-zinc-700 flex items-center gap-1.5 mb-3">
              <Car className="w-3.5 h-3.5 text-auto-500" /> Presión ajustada
            </h3>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-zinc-50 rounded-xl p-3 text-center">
                <p className="text-[10px] text-zinc-400">Delante</p>
                <p className="text-2xl font-black text-auto-600">
                  {resultados.nuevaDelante.toFixed(1)}
                </p>
                <p className="text-[10px] text-zinc-400 mt-0.5">PSI</p>
              </div>
              <div className="bg-zinc-50 rounded-xl p-3 text-center">
                <p className="text-[10px] text-zinc-400">Atrás</p>
                <p className="text-2xl font-black text-auto-600">
          ...
          <div className="card-soft rounded-2xl p-4">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-xs font-bold text-zinc-700">Recomendación</p>
        ...
        <div className="card-soft rounded-2xl p-6 text-center">
          <p className="text-sm text-zinc-400">Completa los campos para ver el ajuste</p>
          <p className="text-xs text-zinc-300 mt-1">Presión en frío y altitudes de origen y destino</p>
        </div>
      )}
      ...
      <details className="card-soft rounded-2xl p-4">
        ...
        <div className="mt-3 space-y-2 text-xs text-zinc-600 bg-zinc-50 rounded-xl p-3">
          <p><code className="text-auto-600 font-bold">Δ Altitud</code> = Altitud Destino − Altitud Origen</p>
          <p><code className="text-auto-600 font-bold">Δ PSI</code> = (Δ Altitud / 300) × 0.5</p>
          <p><code className="text-auto-600 font-bold">Presión Ajustada</code> = Presión en Frío + Δ PSI</p>
          <p className="text-zinc-400 mt-2">Regla de compensación: 0.5 PSI por cada 300 metros de cambio de altitud. Ascenso → suma. Descenso → resta.</p>
        </div>
      </details>
    </div>
  );
