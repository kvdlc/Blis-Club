...
export default function RendimientoClient({ defaults }: { defaults: Defaults }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <h1 className="text-xl font-extrabold text-zinc-800">Comparador de Combustible</h1>
        <p className="text-xs text-zinc-500 mt-1">Descubre qué octanaje te conviene más por tu dinero.</p>
      </div>
      ...
      <div className="card-soft rounded-2xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="text-sm font-bold text-zinc-800 bg-transparent border-b border-dashed border-zinc-300 focus:outline-none focus:border-auto-400"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className="block">
            ...
            <div className="flex gap-1 mt-0.5">
              <input type="number" min="1" step="0.01" value={precioA}
                ...
                className="flex-1 px-2.5 py-2 rounded-lg border border-zinc-200 bg-white text-sm font-medium focus:outline-none focus:ring-2 focus:ring-auto-200" />
              {defaults.ultimosPrecios?.regular && (
                <button onClick={() => usarPrecio("regular", "a")} className="px-2 py-2 rounded-lg bg-zinc-100 text-[10px] font-bold text-zinc-500 hover:bg-auto-100 hover:text-auto-600 transition-colors">
          ...
          <label className="block">
            ...
            <input type="number" min="1" step="0.1" value={rendimientoA}
              ...
              className="w-full mt-0.5 px-2.5 py-2 rounded-lg border border-zinc-200 bg-white text-sm font-medium focus:outline-none focus:ring-2 focus:ring-auto-200" />
          </label>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-4">
        ...
        <span className="text-xs font-black text-zinc-400 bg-zinc-50 px-3 py-1 rounded-full">VS</span>
        <div className="h-px flex-1 bg-zinc-200" />
      </div>
          ...
          <label className="block">
            ...
            <div className="flex gap-1 mt-0.5">
              ...
              {defaults.ultimosPrecios?.premium && (
                <button onClick={() => usarPrecio("premium", "b")} className="px-2 py-2 rounded-lg bg-zinc-100 text-[10px] font-bold text-zinc-500 hover:bg-violet-100 hover:text-violet-600 transition-colors">
      ...
      <div className="card-soft rounded-2xl p-4 space-y-3">
        <h3 className="text-xs font-bold text-zinc-600 flex items-center gap-1.5">
      ...
      {resultados ? (
        <div className="space-y-3">
          {/* Ganador */}
          <div className={`rounded-2xl p-4 text-center border-2 ${resultados.mejor === "a" ? "bg-amber-50 border-amber-200" : "bg-violet-50 border-violet-200"}`}>
            <p className="text-xs text-zinc-500 mb-1">Más económico</p>
          ...
          <div className="card-soft rounded-2xl p-4 space-y-3">
            ...
            <div>
              <div className="flex justify-between text-[10px] mb-1">
                <span className="font-bold text-zinc-700">{nombres.a}</span>
                <span className="font-bold text-zinc-800">S/ {resultados.costoPorKm.a.toFixed(4)}</span>
              </div>
              <div className="h-4 bg-zinc-100 rounded-full overflow-hidden">
                <div className="h-full bg-amber-500 rounded-full flex items-center justify-end pr-2 transition-all duration-500" style={{ width: `${Math.max(resultados.barraA, 5)}%` }}>
            ...
            <div>
              <div className="flex justify-between text-[10px] mb-1">
                <span className="font-bold text-zinc-700">{nombres.b}</span>
                <span className="font-bold text-zinc-800">S/ {resultados.costoPorKm.b.toFixed(4)}</span>
              </div>
              <div className="h-4 bg-zinc-100 rounded-full overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="card-soft rounded-xl p-3 text-center">
              <p className="text-[10px] text-zinc-400">Por tanque lleno</p>
            ...
            <div className="card-soft rounded-xl p-3 text-center">
              <p className="text-[10px] text-zinc-400">Ahorro anual</p>
              <p className="text-base font-black text-zinc-800">
            ...
            <div className="card-soft rounded-xl p-3 text-center">
              <p className="text-[10px] text-zinc-400">Km por tanque (A)</p>
              <p className="text-base font-black text-zinc-800">
                {Math.round(resultados.kmPorTanque.a).toLocaleString("es-PE")}
              </p>
              <p className="text-xs text-zinc-400">{nombres.a}</p>
            </div>
            <div className="card-soft rounded-xl p-3 text-center">
              <p className="text-[10px] text-zinc-400">Km por tanque (B)</p>
              <p className="text-base font-black text-zinc-800">
                {Math.round(resultados.kmPorTanque.b).toLocaleString("es-PE")}
              </p>
              <p className="text-xs text-zinc-400">{nombres.b}</p>
            </div>
          </div>
        </div>
      ) : (
        <div className="card-soft rounded-2xl p-6 text-center">
          <p className="text-sm text-zinc-400">Completa los 4 campos para comparar</p>
          <p className="text-xs text-zinc-300 mt-1">Precio y rendimiento de ambos combustibles</p>
        </div>
      )}

      {/* Fórmula */}
      <details className="card-soft rounded-2xl p-4">
        <summary className="text-xs font-bold text-zinc-500 cursor-pointer">Ver fórmula utilizada</summary>
        <div className="mt-3 space-y-2 text-xs text-zinc-600 bg-zinc-50 rounded-xl p-3">
          <p><code className="text-auto-600 font-bold">Costo por Kilómetro</code> = Precio del Galón / Rendimiento (km/gal)</p>
          <p><code className="text-auto-600 font-bold">Ahorro Anual</code> = |Costo por km A − Costo por km B| × Km por año</p>
          <p><code className="text-auto-600 font-bold">Km por Tanque</code> = Capacidad del tanque × Rendimiento</p>
          <p className="text-zinc-400 mt-1">El combustible con menor costo por kilómetro es el más económico, incluso si su precio por galón es más alto.</p>
        </div>
      </details>
    </div>
  );
