...
const warningLights = [
  ...
  { icon: Cog, name: "Filtro de Partículas (DPF)", severity: "medio", color: "text-zinc-500", bg: "bg-zinc-800", desc: "Filtro de partículas diésel obstruido.", action: "Conduce a velocidad constante en carretera para regenerar." },
        ...
        <div className="bg-zinc-900 border border-white/10 shadow-sm rounded-2xl p-4 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <select value={form.tipo} onChange={(e) => setForm({ ...form, tipo: e.target.value })}
              className="px-2.5 py-2 rounded-lg border border-white/10 text-xs font-medium bg-zinc-800 text-zinc-200">
            ...
            <input type="date" value={form.fecha_emision} onChange={(e) => setForm({ ...form, fecha_emision: e.target.value })}
              className="px-2.5 py-2 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" placeholder="Emisión" />
          </div>
          <input type="date" value={form.fecha_vencimiento} onChange={(e) => setForm({ ...form, fecha_vencimiento: e.target.value })}
            required className="w-full px-2.5 py-2 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
        ...
        <div className="bg-zinc-900 border border-white/10 shadow-sm rounded-2xl p-4 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <input value={form.telefono} onChange={(e) => setForm({ ...form, telefono: e.target.value })}
              placeholder="Teléfono" className="px-2.5 py-2 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
          </div>
          <div className="flex gap-1.5">
            <button onClick={handleAdd} disabled={saving} className="flex-1 px-3 py-1.5 rounded-lg bg-auto-600 text-white text-xs font-bold">
              {saving ? "Guardando..." : "Guardar"}
        ...
        <div className="bg-zinc-900 border border-white/10 shadow-sm rounded-2xl p-4 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <label className="block"><span className="text-[10px] font-bold text-zinc-500">Tipo de aceite</span>
              <input value={form.tipo_aceite} onChange={(e) => setForm({ ...form, tipo_aceite: e.target.value })} placeholder="Ej: Sintético" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
            </label>
            <label className="block"><span className="text-[10px] font-bold text-zinc-500">Viscosidad</span>
              <input value={form.viscosidad_aceite} onChange={(e) => setForm({ ...form, viscosidad_aceite: e.target.value })} placeholder="Ej: 5W-30" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
            </label>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <label className="block"><span className="text-[10px] font-bold text-zinc-500">Cap. aceite (L)</span>
              <input type="number" step="0.1" value={form.capacidad_aceite_litros} onChange={(e) => setForm({ ...form, capacidad_aceite_litros: e.target.value })} placeholder="4.5" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
            </label>
            <label className="block"><span className="text-[10px] font-bold text-zinc-500">Refrigerante</span>
              <input value={form.tipo_refrigerante} onChange={(e) => setForm({ ...form, tipo_refrigerante: e.target.value })} placeholder="Ej: Etilenglicol" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
            </label>
          </div>
          <div className="grid grid-cols-3 gap-2">
            <label className="block"><span className="text-[10px] font-bold text-zinc-500">PSI delante</span>
              <input type="number" value={form.presion_neumaticos_delante} onChange={(e) => setForm({ ...form, presion_neumaticos_delante: e.target.value })} placeholder="32" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
            </label>
            <label className="block"><span className="text-[10px] font-bold text-zinc-500">PSI atrás</span>
              <input type="number" value={form.presion_neumaticos_atras} onChange={(e) => setForm({ ...form, presion_neumaticos_atras: e.target.value })} placeholder="32" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
            </label>
            <label className="block"><span className="text-[10px] font-bold text-zinc-500">PSI repuesto</span>
              <input type="number" value={form.presion_neumaticos_repuesto} onChange={(e) => setForm({ ...form, presion_neumaticos_repuesto: e.target.value })} placeholder="60" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
            </label>
          </div>
          <div className="grid grid-cols-3 gap-2">
            <label className="block"><span className="text-[10px] font-bold text-zinc-500">Tanque (gal)</span>
              <input type="number" step="0.1" value={form.capacidad_tanque_galones} onChange={(e) => setForm({ ...form, capacidad_tanque_galones: e.target.value })} placeholder="14" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
            </label>
            <label className="block"><span className="text-[10px] font-bold text-zinc-500">Cap. refrig. (L)</span>
              <input type="number" step="0.1" value={form.capacidad_refrigerante_litros} onChange={(e) => setForm({ ...form, capacidad_refrigerante_litros: e.target.value })} placeholder="5.0" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
            </label>
            <label className="block"><span className="text-[10px] font-bold text-zinc-500">Líq. frenos</span>
              <input value={form.tipo_freno} onChange={(e) => setForm({ ...form, tipo_freno: e.target.value })} placeholder="DOT 4" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
            </label>
          </div>
          <label className="block"><span className="text-[10px] font-bold text-zinc-500">Octanaje</span>
            <input value={form.octanaje_recomendado} onChange={(e) => setForm({ ...form, octanaje_recomendado: e.target.value })} placeholder="Ej: 95 RON" className="w-full mt-0.5 px-2 py-1.5 rounded-lg border border-white/10 text-xs bg-zinc-800 text-zinc-200" />
          </label>
          <div className="flex gap-1.5">
