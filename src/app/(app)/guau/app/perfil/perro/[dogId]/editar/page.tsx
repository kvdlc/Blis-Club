...
import type { Dog, DogMetabolicProfile, DogMealSlot, WeeklyChallenge, UserChallenge, NutritionRecipe } from "@/types/database";
...
export default async function EditDogPage({ params }: { params: Promise<{ dogId: string }> }) {
  ...
  const [{ data: dog }, { data: mp }, { data: slots }, { data: challenges }, { data: userChallenges }, { data: croquetasRecipes }] = await Promise.all([
    ...
    supabase.from("nutrition_recipes").select("id, title, image_url, protein_type, breed_sizes, category").eq("category", "croquetas").order("title"),
  ]);
  ...
  return (
    <EditDogClient
      ...
      userId={user.id}
      croquetasRecipes={(croquetasRecipes as NutritionRecipe[] | null) ?? []}
    />
  );
}
