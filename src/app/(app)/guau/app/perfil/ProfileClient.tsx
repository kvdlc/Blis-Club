    ...
    <>
      ...
      <div className="card-soft rounded-[1.25rem] p-4 space-y-3">
        ...
        <div className="border-t border-zinc-100 pt-3 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-bold text-zinc-800">Invita y gana</h4>
          </div>
          ...
          {referralCode && (
            <button
              ...
              className="w-full flex items-center justify-between rounded-xl bg-zinc-50 border border-zinc-200 px-3 py-2 transition-colors hover:bg-zinc-100"
            >
              <span className="text-[11px] font-mono text-zinc-600 truncate pr-2">
        ...
        <div className="border-t border-zinc-100 pt-3 space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-xs font-bold text-zinc-800">Tus recompensas</h4>
            </div>
            <Link href="/guau/app/perfil/billetera" className="text-[10px] font-bold text-primary-600 hover:underline">
              Ver billetera →
            </Link>
          </div>
      ...
      <div className="card-soft rounded-[1.5rem] p-5 space-y-3">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-full bg-primary-50 flex items-center justify-center border-2 border-primary-100 overflow-hidden shrink-0">
            <img src={profile?.avatar_url || "/icons/user-default.png"} alt="" className="w-full h-full object-cover object-center" />
          </div>
          <div className="flex-1 min-w-0">
            <h2 className="text-lg font-bold text-zinc-900 truncate">{displayName}</h2>
        ...
        <Link href="/guau/app/perfil/editar"
          className="w-full flex items-center justify-center gap-2 rounded-xl bg-primary-100 text-primary-700 py-2.5 text-sm font-bold active:scale-[0.98] transition-transform">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold text-zinc-800">Mis Perros</h3>
          <Link href="/guau/app/perfil/perro/nuevo" className="flex items-center gap-1 text-xs font-bold text-primary-600">
          ...
          return (
            <div key={dog.id} className="card-soft rounded-[1.5rem] p-4 space-y-3">
              <div className="flex items-start gap-3">
                <div className="w-14 h-14 rounded-2xl bg-white flex items-center justify-center shrink-0 overflow-hidden border border-zinc-200">
                  <img src={dog.foto_url || "/icons/dog-default.png"} alt={dog.nombre} className="w-full h-full object-contain object-center" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-zinc-800">{dog.nombre}</p>
              ...
              <Link href={`/guau/app/perfil/perro/${dog.id}/editar`}
                className="w-full flex items-center justify-center gap-1 rounded-xl bg-primary-100 text-primary-700 py-2 text-xs font-bold active:scale-[0.98] transition-transform">
      ...
      {userBadges.length > 0 && (
        <div className="space-y-2">
          <h3 className="text-sm font-bold text-zinc-800">Insignias</h3>
          <div className="flex flex-wrap gap-3">
            {userBadges.map((ub) => (
              <div key={ub.id} className="flex flex-col items-center gap-1">
                <div className="w-12 h-12 rounded-2xl bg-accent-50 border border-accent-200 flex items-center justify-center">
      ...
      <a href="https://wa.me/1234567890" target="_blank" rel="noopener noreferrer"
        className="flex items-center justify-between card-soft rounded-[1.25rem] p-4 hover:bg-white/90 transition-colors">
        <div className="flex items-center gap-3">
      ...
      <button
        ...
        className="w-full flex items-center justify-between card-soft rounded-[1.25rem] p-4 hover:bg-primary-50 transition-colors"
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <p className="text-sm font-bold text-zinc-800">Ver tutorial de bienvenida</p>
      ...
      <button onClick={handleLogout}
        className="w-full flex items-center justify-center gap-2 card-soft rounded-[1.25rem] p-4 hover:bg-danger-50 transition-colors text-sm font-medium text-danger-600">
