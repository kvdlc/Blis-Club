  ...
  return (
    <div className="space-y-5 pb-8">
      <div className="flex items-center gap-3">
        ...
        {saved && (
          <span className="flex items-center gap-1 text-xs text-secondary-600 bg-secondary-50 dark:bg-secondary-950/30 rounded-full px-2 py-1">
      ...
      <div className="flex flex-col items-center gap-3">
        <label htmlFor="avatarUpload" className="relative w-24 h-24 rounded-full bg-primary-50 dark:bg-primary-950 flex items-center justify-center border-2 border-dashed border-primary-300 dark:border-primary-700 cursor-pointer hover:border-primary-500 transition-colors overflow-hidden group">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div><label className="text-xs text-zinc-500 block mb-1">Nombre</label><input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-sm" placeholder="Kevin" /></div>
        <div><label className="text-xs text-zinc-500 block mb-1">Apellido</label><input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-sm" placeholder="Valdez" /></div>
      </div>

      <div><label className="text-xs text-zinc-500 block mb-1">Correo electrónico</label><input type="email" value={profile?.email ?? ""} disabled className="w-full rounded-xl bg-zinc-100 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-sm text-zinc-400 cursor-not-allowed" /></div>
      ...
      {subscription && (
        <div className="card-soft rounded-[1.25rem] p-4 space-y-1">
          <p className="text-sm font-bold text-zinc-800 dark:text-zinc-200">Plan {(subscription.plans as Plan)?.name ?? "Free"}</p>
      ...
      <div className="space-y-2">
        <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-200">Seguridad</h3>
        {pwSuccess && <p className="text-xs text-secondary-600 font-semibold bg-secondary-50 dark:bg-secondary-950/20 rounded-xl p-2 text-center">¡Contraseña actualizada!</p>}
        <button onClick={() => { setShowCreatePw(!showCreatePw); setShowChangePw(false); }} className="w-full flex items-center justify-between card-soft rounded-[1.25rem] p-4 text-sm text-primary-600 dark:text-primary-400 font-medium">
        ...
        {showCreatePw && (
          <div className="space-y-2 p-4 rounded-xl bg-primary-50/30 dark:bg-primary-950/20">
            <p className="text-[10px] text-zinc-500">Si ingresaste con Magic Link y no tienes contraseña</p>
            <input type="password" value={newPw} onChange={(e) => setNewPw(e.target.value)} placeholder="Nueva contraseña" className="w-full rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-sm" />
            <input type="password" value={confirmPw} onChange={(e) => setConfirmPw(e.target.value)} placeholder="Confirmar" className="w-full rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-sm" />
        ...
        <button onClick={() => { setShowChangePw(!showChangePw); setShowCreatePw(false); }} className="w-full flex items-center justify-between card-soft rounded-[1.25rem] p-4 text-sm text-zinc-600 dark:text-zinc-400">
        ...
        {showChangePw && (
          <div className="space-y-2 p-4 rounded-xl bg-zinc-50 dark:bg-zinc-800/30">
            <input type="password" value={currentPw} onChange={(e) => setCurrentPw(e.target.value)} placeholder="Contraseña actual" className="w-full rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-sm" />
            <input type="password" value={newPw} onChange={(e) => setNewPw(e.target.value)} placeholder="Nueva contraseña" className="w-full rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-sm" />
            <input type="password" value={confirmPw} onChange={(e) => setConfirmPw(e.target.value)} placeholder="Confirmar" className="w-full rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-sm" />
            {pwError && <p className="text-xs text-danger-600">{pwError}</p>}
