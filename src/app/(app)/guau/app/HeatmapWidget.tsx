...
function getTextColor(light: string | null) {
  switch (light) {
    case "green": return "text-secondary-600";
    case "yellow": return "text-warning-600";
    case "red": return "text-danger-600";
  ...
  return (
    <div className="card-soft rounded-[1.5rem] p-5 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-zinc-800">Últimos paseos</h3>
        <span className="text-[10px] font-semibold text-zinc-500">{greenPct}% en verde · {todayWalks} hoy</span>
      </div>
        ...
        {weekDays.map((d) => {
          ...
          return (
            <button
              ...
              className={`flex-1 h-12 rounded-xl transition-all ${color} flex flex-col items-center justify-center gap-0.5 ${ isSelected ? "ring-2 ring-primary-500 ring-offset-2" : "" } ${isToday && !isSelected ? "ring-2 ring-white/60" : ""}`}
              title={d.toLocaleDateString("es", { weekday: "short", day: "numeric" })}
            >
              <span className={`text-[10px] font-bold block text-center ${hasWalks ? "text-white" : "text-zinc-500"}`}>
                {d.toLocaleDateString("es", { weekday: "narrow" })}
              </span>
              {hasWalks && (
                <div className="flex gap-0.5">
                  {walksByDay[dateStr]!.slice(0, 3).map((w, i) => (
                    <div key={i} className={`w-1 h-1 rounded-full ${ w.traffic_light === "green" ? "bg-white/80" : w.traffic_light === "yellow" ? "bg-white/60" : "bg-white/40" }`} />
                  ))}
                </div>
              )}
            </button>
      ...
      {selectedDate && selectedWalks.length > 0 && (
        <div className="rounded-xl bg-zinc-50 p-3 space-y-1.5">
          ...
          <div className="flex items-center gap-3 text-sm flex-wrap">
            <span className="flex items-center gap-1 text-zinc-600"><Footprints className="w-3.5 h-3.5" /> {selectedWalks.length} paseo{selectedWalks.length !== 1 ? "s" : ""}</span>
            {selPipis > 0 && <span className="flex items-center gap-1 text-zinc-600"><Droplets className="w-3.5 h-3.5 text-warning-500" /> {selPipis}</span>}
            {selPopos > 0 && <span className="flex items-center gap-1 text-zinc-600"><BadgeCheck className="w-3.5 h-3.5 text-secondary-500" /> {selPopos}</span>}
            <span className="flex items-center gap-1 text-zinc-600"><Clock className="w-3.5 h-3.5" /> {Math.round(selMins)}min</span>
      ...
      {selectedDate && selectedWalks.length === 0 && (
        <p className="text-sm text-zinc-400 text-center py-2">Sin paseos este día</p>
      )}
    </div>
  );
}
