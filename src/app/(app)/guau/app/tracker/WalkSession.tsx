...
export function WalkSession({ allDogs, userId, onDone, onClose }: Props) {
  ...
  return (
    <div className="card-soft rounded-[1.5rem] p-5 space-y-5 bg-white border-2 border-primary-200">
      {/* ═══ SELECTING ═══ */}
      {mainPhase === "selecting" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-zinc-800">¿Quién va al paseo?</h3>
            ...
            {allDogs.map((dog) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-3 rounded-xl border-2 transition-all ${ selected ? "border-primary-400 bg-primary-50" : "border-zinc-100" }`}
                >
                  <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center ${ selected ? "bg-primary-500 border-primary-500" : "border-zinc-300" }`}>
                    {selected && <Check className="w-3 h-3 text-white" />}
                  </div>
                  <span className="text-sm font-semibold text-zinc-700">{dog.nombre}</span>
                </button>
              );
            })}
          </div>
      ...
      {mainPhase === "active" && (
        <div className="space-y-6">
          <div className="flex flex-col items-center py-2">
            <p className="text-5xl font-bold tabular-nums text-primary-600">
          ...
          {allDogs.filter((d) => selectedDogIds.includes(d.id)).map((dog) => (
            <div key={dog.id} className="bg-zinc-50 rounded-2xl p-4 space-y-3">
              <p className="text-sm font-bold text-zinc-700">{dog.nombre}</p>
              <div className="flex gap-3">
                <button onClick={() => addPipi(dog.id)}
                  className="flex-1 flex flex-col items-center gap-1 bg-warning-50 border border-warning-200 rounded-xl p-3 active:scale-95 transition-transform">
                ...
                <button onClick={() => addPopo(dog.id)}
                  className="flex-1 flex flex-col items-center gap-1 bg-secondary-50 border border-secondary-200 rounded-xl p-3 active:scale-95 transition-transform">
      ...
      {mainPhase === "evaluating" && currentDog && (
        <div className="space-y-5">
          <div className="text-center">
            ...
            <h3 className="text-lg font-bold text-zinc-900 mt-1">{currentDog.nombre}</h3>
          </div>
              ...
              <div className="flex gap-4 justify-center">
                {[
                  ...
                  <button key={opt.value} onClick={() => handleEvalTraffic(opt.value)}
                    className="flex flex-col items-center gap-3 p-5 rounded-2xl bg-white border-2 border-zinc-200 hover:scale-105 active:scale-95 transition-all">
                ...
                {TRIGGER_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button key={opt.tag} onClick={() => toggleTrigger(opt.tag)}
                      ...
                      <span className={`text-[10px] font-semibold leading-tight text-center ${selected ? "text-primary-700" : "text-zinc-600"}`}>{opt.tag}</span>
                    </button>
                  );
                })}
                <button onClick={() => setShowCustomTrigger(!showCustomTrigger)}
                  className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all active:scale-95 ${ showCustomTrigger ? "border-primary-400 bg-primary-50" : "border-dashed border-zinc-300 bg-white/40" }`}>
              ...
              {showCustomTrigger && (
                <div className="flex gap-2">
                  <input type="text" value={customTrigger} onChange={(e) => setCustomTrigger(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addCustomTrigger()}
                    ...
                    className="flex-1 rounded-xl bg-white border border-zinc-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20" autoFocus />
              ...
              <div className="flex gap-2 justify-center">
                {[
                  ...
                  <button key={s.n} onClick={() => handleEvalStool(s.n)}
                    className="flex flex-col items-center gap-1 p-3 rounded-2xl transition-all bg-zinc-50 border-2 border-transparent hover:border-primary-400 active:scale-95">
      ...
      {mainPhase === "done" && (
        <div className="space-y-4 text-center">
          ...
          <h3 className="text-lg font-bold text-secondary-600">¡Paseo guardado!</h3>
          ...
          <div className="space-y-2 text-left">
            {allDogs.filter((d) => selectedDogIds.includes(d.id)).map((dog) => {
              ...
              return (
                <div key={dog.id} className="bg-zinc-50 rounded-xl p-3 text-xs">
                  <p className="font-bold text-zinc-700">{dog.nombre}</p>
