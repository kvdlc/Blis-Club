...
export function VaccineCard({ vaccine, records, doses, onOpenWiki }: Props) {
  ...
  return (
    <div className={`card-soft rounded-2xl flex items-center gap-3 p-4 transition-all overflow-hidden ${borderColor}`}>
      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0 bg-primary-100">
        {vaccine.emoji}
      </div>

      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="text-sm font-bold text-zinc-800 truncate">{vaccine.name}</p>
          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full shrink-0 ${vaccine.group === "core" ? "bg-primary-100 text-primary-700" : "bg-zinc-100 text-zinc-600"}`}>
          ...
          {doses.map((d, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-1">
                <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 transition-all ${ isDone ? "bg-secondary-500 text-white shadow-sm shadow-secondary-500/30" : "bg-primary-100 text-primary-500" }`}>
                  {d.doseNumber}
                </div>
                {i < doses.length - 1 && (
                  <div className={`w-2.5 h-0.5 rounded-full transition-colors ${isDone ? "bg-secondary-400" : "bg-primary-200"}`} />
                )}
              </div>
            );
          })}
      ...
      <button
        ...
        className="shrink-0 w-8 h-8 rounded-full bg-accent-100 flex items-center justify-center hover:bg-accent-200 transition-colors self-start"
        title="Más info"
      >
