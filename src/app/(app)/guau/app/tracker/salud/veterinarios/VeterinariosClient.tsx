    ...
    <div className="space-y-4 pb-8">
      <div className="flex items-center gap-3 pt-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-zinc-900">Veterinarios de Confianza</h1>
        </div>
      </div>
      ...
      <div className="space-y-3">
          ...
          <div key={vet.id} className="card-soft rounded-[1.25rem] p-4 space-y-2">
            {editing === vet.id ? (
              <div className="space-y-2">
                <input value={editForm.name} onChange={(e) => setEditForm((f) => ({ ...f, name: e.target.value }))} placeholder="Nombre" className="w-full rounded-lg bg-zinc-50 border border-zinc-200 px-3 py-1.5 text-xs" />
                <input value={editForm.clinic} onChange={(e) => setEditForm((f) => ({ ...f, clinic: e.target.value }))} placeholder="Clínica" className="w-full rounded-lg bg-zinc-50 border border-zinc-200 px-3 py-1.5 text-xs" />
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {SPECIALTIES.map((s) => (
                      <button key={s} type="button" onClick={() => setEditForm((f) => ({ ...f, specialty: f.specialty === s ? "" : s }))}
                        className={`text-[10px] rounded-full px-2.5 py-1 transition-colors ${editForm.specialty === s ? "bg-accent-500 text-white" : "bg-zinc-100 text-zinc-600"}`}>
                        {s}
                      </button>
                    ))}
                  </div>
              ...
              <>
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-zinc-800 truncate">{vet.name}</p>
                    {vet.clinic_name && <p className="text-xs text-zinc-500 truncate">🏥 {vet.clinic_name}</p>}
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => startEdit(vet)} className="w-6 h-6 rounded-full bg-zinc-100 flex items-center justify-center hover:bg-zinc-200 transition-colors">
                      <Pencil className="w-3 h-3 text-zinc-500" />
                    </button>
                    <button onClick={() => deleteVet(vet.id)} className="w-6 h-6 rounded-full bg-danger-100 flex items-center justify-center hover:bg-danger-200 transition-colors">
                ...
                {vet.specialty && (
                  <span className="text-[10px] bg-accent-50 text-accent-700 px-2 py-0.5 rounded-full inline-block">
                    🎯 {vet.specialty}
                  </span>
                )}
                ...
                {vet.phone && (
                  <div className="flex items-center gap-2 pt-1">
                    <a
                      ...
                      className="flex items-center gap-1.5 bg-secondary-100 text-secondary-700 rounded-full px-3 py-1.5 text-[11px] font-semibold hover:bg-secondary-200 transition-colors"
                    >
                    ...
                    <a
                      ...
                      className="flex items-center gap-1.5 bg-emerald-100 text-emerald-700 rounded-full px-3 py-1.5 text-[11px] font-semibold hover:bg-emerald-200 transition-colors"
                    >
        ...
        {!showAddVet && (
          <button onClick={() => setShowAddVet(true)} className="w-full flex items-center justify-center gap-1 card-soft rounded-[1.25rem] p-3 border-2 border-dashed border-zinc-300 text-xs text-zinc-500 font-semibold hover:bg-zinc-50 transition-colors">
            <Plus className="w-4 h-4" /> Agregar Veterinario
          </button>
        )}

        {showAddVet && (
          <form onSubmit={addTrustedVet} className="card-soft rounded-[1.25rem] p-5 space-y-3 bg-gradient-to-br from-accent-50/50 to-secondary-50/50 border-2 border-dashed border-accent-200">
            <div className="flex items-center gap-2">
              <GraduationCap className="w-4 h-4 text-accent-500" />
              <span className="text-xs font-bold text-accent-700">Nuevo Veterinario</span>
            </div>
            ...
            <div>
              ...
              <input name="name" placeholder="Dr(a). Nombre completo *" required className="w-full rounded-xl bg-zinc-50 border border-zinc-200 px-3 py-2 text-xs" />
            </div>
            ...
            <div>
              ...
              <input name="clinic" placeholder="Nombre de la clínica" className="w-full rounded-xl bg-zinc-50 border border-zinc-200 px-3 py-2 text-xs" />
            </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {SPECIALTIES.map((s) => (
                  <button key={s} type="button" onClick={() => setSpecialty(specialty === s ? "" : s)}
                    className={`text-[10px] rounded-full px-2.5 py-1 transition-colors ${specialty === s ? "bg-accent-500 text-white shadow-sm" : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200"}`}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => { setShowAddVet(false); resetForm(); }} className="flex-1 bg-zinc-100 rounded-xl py-2.5 text-xs font-semibold">
                Cancelar
              </button>
            </div>
          </form>
