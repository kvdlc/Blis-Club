    ...
    <div className="space-y-4 pb-8">
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        <div className="flex items-center gap-2 flex-1">
          ...
          <h1 className="text-lg font-bold text-zinc-900">Medicamentos</h1>
        </div>
        <button
          onClick={() => setShowForm(!showForm)}
          className={`flex items-center gap-1.5 rounded-full px-4 py-2 text-xs font-bold transition-all ${ showForm ? "bg-warning-100 text-warning-700" : "bg-warning-600 text-white hover:bg-warning-700 shadow-md active:scale-95" }`}
        >
      ...
      {showForm && (
        <form onSubmit={addMedication} className="card-soft rounded-[1.25rem] p-5 space-y-3 bg-gradient-to-br from-warning-50/50 to-primary-50/50 border-2 border-dashed border-warning-200">
          <div className="flex items-center gap-2">
            <Pill className="w-4 h-4 text-warning-500" />
            <span className="text-xs font-bold text-warning-700">Nuevo Medicamento</span>
          </div>
          <div className="flex gap-2">
            <div className="flex-1"><span className="text-xs text-zinc-400 block mb-1">💊 Nombre</span><input name="medication_name" placeholder="Nombre *" required className="w-full rounded-xl bg-zinc-50 border border-zinc-200 px-3 py-2 text-xs" /></div>
            <div className="flex-1"><span className="text-xs text-zinc-400 block mb-1">⚗️ Dosis</span><input name="dosage" placeholder="Ej: 1 comp/8h" className="w-full rounded-xl bg-zinc-50 border border-zinc-200 px-3 py-2 text-xs" /></div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={() => setIntervalDays(Math.max(1, intervalDays - 1))} className="w-7 h-7 rounded-lg bg-zinc-100 flex items-center justify-center"><Minus className="w-3.5 h-3.5 text-zinc-500" /></button>
              <input type="number" min={1} value={intervalDays} onChange={(e) => setIntervalDays(Math.max(1, parseInt(e.target.value) || 1))} className="w-14 text-center rounded-lg bg-zinc-50 border border-zinc-200 px-2 py-2 text-sm font-bold text-zinc-800" />
              <button type="button" onClick={() => setIntervalDays(intervalDays + 1)} className="w-7 h-7 rounded-lg bg-zinc-100 flex items-center justify-center"><Plus className="w-3.5 h-3.5 text-zinc-500" /></button>
            ...
            <div className="flex gap-1.5">
              {[1, 2, 3, 4].map((n) => (
                <button key={n} type="button" onClick={() => setDosesPerDay(n)}
                  className={`flex-1 text-xs font-semibold rounded-lg py-2 transition-colors ${dosesPerDay === n ? "bg-warning-500 text-white shadow-sm" : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200"}`}>{n}</button>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-4 gap-1">
            {[0, 1, 2, 3].map((i) => (
              <div key={i}><input type="time" name={`dose_time_${i}`} defaultValue={["08:00", "14:00", "20:00", "00:00"][i]} className={`w-full rounded-lg bg-zinc-50 border border-zinc-200 px-2 py-1 text-xs ${i >= dosesPerDay ? "opacity-30" : ""}`} disabled={i >= dosesPerDay} /></div>
            ))}
          </div>
          ...
          <div key={med.id} className={`card-soft rounded-[1.25rem] p-4 space-y-2 ${med.status === "completed" ? "opacity-60" : ""}`}>
            {editing === med.id ? (
              <div className="space-y-2">
                <input value={editForm.medication_name || ""} onChange={(e) => setEditForm((f) => ({ ...f, medication_name: e.target.value }))} placeholder="Nombre" className="w-full rounded-lg bg-zinc-50 border border-zinc-200 px-3 py-1.5 text-xs" />
                <input value={editForm.dosage || ""} onChange={(e) => setEditForm((f) => ({ ...f, dosage: e.target.value }))} placeholder="Dosis" className="w-full rounded-lg bg-zinc-50 border border-zinc-200 px-3 py-1.5 text-xs" />
                <div className="flex flex-col sm:flex-row gap-2">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-zinc-500">Cada</span>
                    <button type="button" onClick={() => editIntervalDays(intervalEdit - 1)} className="w-6 h-6 rounded-lg bg-zinc-100 flex items-center justify-center"><Minus className="w-3 h-3 text-zinc-500" /></button>
                    <input type="number" min={1} value={intervalEdit} onChange={(e) => editIntervalDays(parseInt(e.target.value) || 1)} className="w-12 text-center rounded-lg bg-zinc-50 border border-zinc-200 px-2 py-1.5 text-xs font-bold text-zinc-800" />
                    <button type="button" onClick={() => editIntervalDays(intervalEdit + 1)} className="w-6 h-6 rounded-lg bg-zinc-100 flex items-center justify-center"><Plus className="w-3 h-3 text-zinc-500" /></button>
                    <span className="text-xs text-zinc-500">días</span>
                  </div>
                </div>
                <div><span className="text-[10px] text-zinc-400 block mb-0.5">🕐 Dosis por día</span>
                  <div className="flex gap-1.5">{[1, 2, 3, 4].map((n) => (<button key={n} type="button" onClick={() => editDoses(n)} className={`flex-1 text-xs font-semibold rounded-lg py-1.5 transition-colors ${dosesEdit === n ? "bg-warning-500 text-white shadow-sm" : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200"}`}>{n}</button>))}</div>
                </div>
                <div><span className="text-[10px] text-zinc-400 block mb-0.5">⏰ Horarios</span>
                  <div className="grid grid-cols-4 gap-1">{[0, 1, 2, 3].map((i) => (<div key={i}><input type="time" value={(hoursEdit[i] || "").slice(0, 5)} onChange={(e) => editHour(i, e.target.value)} className={`w-full rounded-lg bg-zinc-50 border border-zinc-200 px-2 py-1 text-xs ${i >= dosesEdit ? "opacity-30" : ""}`} disabled={i >= dosesEdit} /></div>))}</div>
                </div>
                <div className="flex gap-2">
              ...
              <>
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-zinc-800 truncate">{med.medication_name}</p>
                    {med.dosage && <p className="text-xs text-zinc-500 truncate">{med.dosage}</p>}
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => startEdit(med)} className="w-6 h-6 rounded-full bg-zinc-100 flex items-center justify-center hover:bg-zinc-200"><Pencil className="w-3 h-3 text-zinc-500" /></button>
                    <button onClick={() => deleteMed(med.id)} className="w-6 h-6 rounded-full bg-danger-100 flex items-center justify-center hover:bg-danger-200"><Trash2 className="w-3 h-3 text-danger-500" /></button>
                ...
                {med.dose_hours && med.dose_hours.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {med.dose_hours.map((h, i) => (<div key={i} className="flex items-center gap-1 bg-zinc-100 rounded-lg px-2 py-1 text-[10px]"><Clock className="w-2.5 h-2.5" /> {h.slice(0, 5)}</div>))}
                  </div>
                )}
                  ...
                  <div className="mt-2 pt-2 border-t border-zinc-100 space-y-2">
                    ...
                    {doseInfo.due ? (
                      <div className="bg-warning-50 rounded-xl p-3 space-y-2">
                        <p className="text-[10px] font-semibold text-warning-700">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <input type="time" value={getDefaultDoseTime(med)}
                            ...
                            className="rounded-lg bg-white border border-warning-200 px-2 py-1 text-xs font-mono" />
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => markDose(med, false)}
                            className="flex-1 bg-zinc-200 text-zinc-600 rounded-lg py-1.5 text-xs font-semibold flex items-center justify-center gap-1 hover:bg-zinc-300 transition-colors">
                            <XCircle className="w-3 h-3" /> Omitir
                          </button>
                        </div>
                      </div>
                    ) : doseInfo.takenToday ? (
                      <p className="text-[10px] text-secondary-600 text-center py-1">✅ Dosis de hoy registrada</p>
                    ) : (
                    ...
                    {doseInfo.history.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {doseInfo.history.map((log, i) => (
                          <div key={i} className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] ${log.taken ? "bg-secondary-50 text-secondary-700" : "bg-danger-50 text-danger-600"}`}>
                    ...
                    <button onClick={() => stopTreatment(med.id)}
                      className="w-full flex items-center justify-center gap-1.5 rounded-lg border border-zinc-200 py-1.5 text-[10px] font-semibold text-zinc-400 hover:text-danger-500 hover:border-danger-200 transition-colors">
