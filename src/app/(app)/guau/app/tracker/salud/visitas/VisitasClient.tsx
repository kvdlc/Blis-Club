    ...
    <div className="space-y-4 pb-8">
      <div className="flex items-center gap-3 pt-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-zinc-900">Visitas al Veterinario</h1>
        </div>
      </div>
      ...
      <div className="space-y-3">
          ...
          <div key={visit.id} className="card-soft rounded-[1.25rem] p-4 space-y-1">
            {editing === visit.id ? (
              <div className="space-y-2">
                <input value={editForm.motivo || ""} onChange={(e) => setEditForm((f) => ({ ...f, motivo: e.target.value }))} placeholder="Motivo" className="w-full rounded-lg bg-zinc-50 border border-zinc-200 px-3 py-1.5 text-xs" />
                <input value={editForm.vet_name || ""} onChange={(e) => setEditForm((f) => ({ ...f, vet_name: e.target.value }))} placeholder="Veterinario" className="w-full rounded-lg bg-zinc-50 border border-zinc-200 px-3 py-1.5 text-xs" />
                <div className="flex gap-2">
                  <input type="number" step="0.1" value={editForm.peso_kg?.toString() || ""} onChange={(e) => setEditForm((f) => ({ ...f, peso_kg: parseFloat(e.target.value) || null }))} placeholder="Peso kg" className="w-24 rounded-lg bg-zinc-50 border border-zinc-200 px-3 py-1.5 text-xs" />
                  <DatePicker value={editForm.fecha || ""} onChange={(d) => setEditForm((f) => ({ ...f, fecha: d }))} />
                </div>
                <input value={editForm.diagnostico || ""} onChange={(e) => setEditForm((f) => ({ ...f, diagnostico: e.target.value }))} placeholder="Diagnóstico" className="w-full rounded-lg bg-zinc-50 border border-zinc-200 px-3 py-1.5 text-xs" />
                <div className="flex gap-2">
              ...
              <>
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-zinc-800 truncate min-w-0 flex-1">{visit.motivo}</p>
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => startEdit(visit)} className="w-6 h-6 rounded-full bg-zinc-100 flex items-center justify-center hover:bg-zinc-200 transition-colors">
                      <Pencil className="w-3 h-3 text-zinc-500" />
                    </button>
                    <button onClick={() => deleteVisit(visit.id)} className="w-6 h-6 rounded-full bg-danger-100 flex items-center justify-center hover:bg-danger-200 transition-colors">
                ...
                <div className="flex items-center gap-1 pt-1">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <button key={s} onClick={() => rateVetVisit(visit.id, s)}
                      className={`text-sm ${(visit.vet_rating ?? 0) >= s ? "text-warning-400" : "text-zinc-300"}`}>
        ...
        <form onSubmit={addVetVisit} className="card-soft rounded-[1.25rem] p-5 space-y-3 bg-gradient-to-br from-secondary-50/50 to-primary-50/50 border-2 border-dashed border-secondary-200">
          <div className="flex items-center gap-2">
            <ClipboardList className="w-4 h-4 text-secondary-500" />
            <span className="text-xs font-bold text-secondary-700">Nueva Visita</span>
          </div>
          ...
          <div>
            <div className="flex items-center gap-1 mb-1"><span className="text-xs text-zinc-500">📝 Motivo</span></div>
            <input name="motivo" placeholder="Ej: Revisión general, vacuna..." required className="w-full rounded-xl bg-zinc-50 border border-zinc-200 px-3 py-2 text-xs" />
          </div>

          <div className="flex gap-2">
            <div className="flex-1">
              <div className="flex items-center gap-1 mb-1"><span className="text-xs text-zinc-500">👨‍⚕️ Veterinario</span></div>
              <input name="vet_name" placeholder="Nombre del vet" className="w-full rounded-xl bg-zinc-50 border border-zinc-200 px-3 py-2 text-xs" />
            </div>
            <div className="w-24">
              <div className="flex items-center gap-1 mb-1"><span className="text-xs text-zinc-500">⚖️ Peso</span></div>
              <input name="peso_kg" type="number" step="0.1" placeholder="kg" className="w-full rounded-xl bg-zinc-50 border border-zinc-200 px-3 py-2 text-xs" />
            </div>
          </div>

          <div>
            <div className="flex items-center gap-1 mb-1"><span className="text-xs text-zinc-500">🩺 Veterinario de confianza</span></div>
          ...
          <div>
            ...
            <textarea name="notas" placeholder="Notas de la visita" className="w-full rounded-xl bg-zinc-50 border border-zinc-200 px-3 py-2 text-xs" rows={2} />
          </div>

          <div>
            <div className="flex items-center gap-1 mb-1"><span className="text-xs text-zinc-500">📋 Diagnóstico</span></div>
            <textarea name="diagnostico" placeholder="Diagnóstico del veterinario" className="w-full rounded-xl bg-zinc-50 border border-zinc-200 px-3 py-2 text-xs" rows={2} />
          </div>
