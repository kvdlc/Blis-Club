...
export default function RoutineBuilder({ userId, onClose, onSave }: RoutineBuilderProps) {
  ...
  const [expandedMuscles, setExpandedMuscles] = useState<Set<string>>(new Set(["Pecho"]));
    ...
    const load = async () => {
      ...
      const [favRes, allRes] = await Promise.all([
        supabase
          ...
          .select(
            "exercise_id, spartan_exercise_configs!inner(*), spartan_exercise_library!inner(*)"
          )
          .eq("user_id", userId),
        supabase
      ...
      const favData = ((favRes.data ?? []) as any[]).map((row) => ({
        exercise: row.spartan_exercise_library as Exercise,
        config: {
          default_sets: row.spartan_exercise_configs?.default_sets ?? 3,
          default_reps: row.spartan_exercise_configs?.default_reps ?? 10,
          default_weight_kg: row.spartan_exercise_configs?.default_weight_kg ?? null,
          default_rest_seconds: row.spartan_exercise_configs?.default_rest_seconds ?? 60,
          machine_name: row.spartan_exercise_configs?.machine_name ?? null,
        } as ExerciseConfig,
      }));

      setFavorites(favData);
      setAllExercises((allRes.data ?? []) as Exercise[]);
      setLoading(false);
    };
    load();
  }, [userId]);
  ...
  const addExercise = (exercise: Exercise, config?: ExerciseConfig) => {
    ...
    setSelected((prev) => [
      ...prev,
      {
        exercise,
        config: {
          sets: config?.default_sets ?? 3,
          reps: config?.default_reps ?? 10,
          weight: config?.default_weight_kg ?? null,
          rest: config?.default_rest_seconds ?? 60,
        },
      },
    ]);
  };
  ...
  const updateSelectedConfig = (
    idx: number,
    field: keyof SelectedExercise["config"],
    value: number | null
  ) => {
    setSelected((prev) =>
      prev.map((s, i) =>
        i === idx ? { ...s, config: { ...s.config, [field]: value } } : s
      )
    );
  };
  ...
  const handleSave = async () => {
    ...
    const { data: routine } = await supabase
      ...
      .insert({
        ...
        muscle_group: muscleTarget || null,
        is_active: true,
      })
      .select("id")
      .single();
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 pb-48">
        ...
        <div className="flex bg-white/[0.03] border border-white/[0.06] rounded-xl p-1">
          ...
          >
            <Star className="w-3 h-3" /> Mis ejercicios
          </button>
          <button
            onClick={() => setTab("all")}
              ...
              <div className="space-y-2">
                {favorites.length === 0 ? (
                  <div className="text-center py-10">
                    ...
                    <p className="text-xs text-zinc-500">No tienes ejercicios favoritos</p>
                    ...
                    {favorites.map((fav) => (
                      ...
                      >
                        <div className="aspect-square bg-zinc-900 overflow-hidden">
                          <img
                            src={fav.exercise.gif_url}
                            alt={fav.exercise.name}
                        ...
                        <div className="p-2.5 space-y-1.5">
                          ...
                          <p className="text-[10px] text-zinc-500">
                            {fav.config.default_sets}x{fav.config.default_reps}
                            {fav.config.default_weight_kg
                              ? ` @ ${fav.config.default_weight_kg}kg`
                              : ""}
                          </p>
                          <button
                  ...
                  return (
                    <div key={muscle} className="overflow-hidden">
                      <button
                        ...
                        className="w-full flex items-center justify-between px-3 py-2.5 bg-white/[0.03] border border-white/[0.06] rounded-xl hover:bg-white/[0.05] transition-colors"
                      >
                        <span className="text-xs font-bold text-zinc-300">
                          {muscle}
                          <span className="text-zinc-600 ml-1.5 text-[10px] font-normal">
                            {exercises.length}
                          </span>
                        </span>
                        {expanded ? (
                          ...
                          >
                            <div className="space-y-1 pt-1 px-1">
                              {exercises.map((ex) => (
                                <div
                                  key={ex.id}
                                  className="flex items-center gap-3 p-2 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.04] transition-colors"
                                >
                                  <img
                                    src={ex.gif_url}
                                    alt={ex.name}
      ...
      {/* Bottom panel: selected exercises */}
      <div className="shrink-0 border-t border-white/[0.06] bg-zinc-950/95 backdrop-blur-sm max-h-[60vh] flex flex-col">
        ...
        {selected.length === 0 ? (
          <div className="py-10 text-center">
            <p className="text-xs text-zinc-600">Agrega ejercicios para crear tu rutina</p>
          </div>
        ) : (
                    ...
                    >
                      <div className="flex items-center gap-1 shrink-0">
                        <button
                          ...
                          disabled={idx === 0}
                          className="w-5 h-5 rounded flex items-center justify-center text-zinc-600 hover:text-zinc-400 disabled:opacity-20"
                        >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-zinc-500">
                          {s.config.sets}x{s.config.reps}
                          {s.config.weight ? ` @ ${s.config.weight}kg` : ""}
                        </p>
                      </div>
                      <button
                        onClick={(e) => {
                          ...
                          <div className="px-3 pb-3 grid grid-cols-2 gap-2">
                            <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-2">
                              ...
                              <input
                                ...
                                value={s.config.sets}
                                onChange={(e) =>
                                  updateSelectedConfig(idx, "sets", parseInt(e.target.value) || 1)
                                }
                            ...
                            <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-2">
                              ...
                              <input
                                ...
                                value={s.config.reps}
                                onChange={(e) =>
                                  updateSelectedConfig(idx, "reps", parseInt(e.target.value) || 1)
                                }
                                className="w-full bg-transparent text-sm font-bold text-white text-center focus:outline-none"
                              />
                            </div>
                            <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-2">
                              ...
                              <input
                                ...
                                placeholder="--"
                                className="w-full bg-transparent text-sm font-bold text-white text-center focus:outline-none placeholder:text-zinc-600"
                              />
                            </div>
                            <div className="bg-white/[0.02] border border-white/[0.05] rounded-xl p-2">
