  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <div className="flex bg-white/[0.03] border border-white/[0.06] rounded-xl p-1">
        <button onClick={() => setShowFavoritesOnly(false)} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${!showFavoritesOnly ? "bg-red-600 text-white shadow-[0_0_15px_rgba(220,38,38,0.4)]" : "text-zinc-500 hover:text-zinc-300"}`}>
          Todos
        </button>
        <button onClick={() => setShowFavoritesOnly(true)} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1 ${showFavoritesOnly ? "bg-red-600 text-white shadow-[0_0_15px_rgba(220,38,38,0.4)]" : "text-zinc-500 hover:text-zinc-300"}`}>
      ...
      <div className="relative">
        ...
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar ejercicio..." className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-white/[0.06] bg-white/[0.03] text-sm font-medium text-white placeholder:text-zinc-600 focus:outline-none focus:border-red-500/50" />
      ...
      <div className="grid grid-cols-2 gap-2">
        {filtered.map((ex, i) => (
          <motion.button
            ...
            className="group bg-white/[0.03] border border-white/[0.06] rounded-2xl overflow-hidden hover:border-red-500/30 hover:shadow-[0_0_20px_rgba(220,38,38,0.1)] transition-all active:scale-[0.98] text-left"
          >
