...
import { ArrowLeft, Clock, Check, Circle, ChevronRight, Plus, Trophy } from "lucide-react";
  ...
  return (
    <div className="relative w-36 h-36 flex items-center justify-center">
      <svg className="absolute inset-0 -rotate-90" viewBox="0 0 120 120">
        ...
        <circle cx="60" cy="60" r={radius} fill="none" stroke="url(#timerGradient)" strokeWidth="6" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={offset} className="transition-all duration-1000 ease-linear" />
        <defs>
...
export default function WorkoutSession({ userId, routineId, routineName, exercises, onClose }: WorkoutSessionProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (data.routineId === routineId && data.workoutStarted) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({
        routineId, currentExerciseIndex,
        completedExercises: Array.from(completedExercises),
        allSeriesData, elapsedTime, workoutStarted: workoutStarted.toISOString(),
      }));
    } catch {}
  }, [routineId, currentExerciseIndex, completedExercises, allSeriesData, elapsedTime]);
  ...
  // Update a single series field (weight or reps) without marking complete
  ...
  const completeCurrentExercise = () => {
    ...
    // Calculate actual rest time if rest timer was running
    if (restStartTime) {
      const actualRest = Math.round((Date.now() - restStartTime) / 1000);
      setAllSeriesData((prev) => {
        const next = { ...prev };
        const current = next[currentExerciseIndex];
        if (current) next[currentExerciseIndex] = { ...current, actualRestSeconds: actualRest };
        return next;
      });
  ...
  const advanceToNext = useCallback(() => {
    if (restStartTime) {
      ...
      setAllSeriesData((prev) => {
        ...
        const current = next[prevIdx];
        if (current) next[prevIdx] = { ...current, actualRestSeconds: actualRest };
        return next;
      });
      setRestStartTime(null);
    }
  ...
  const finishWorkout = async () => {
    ...
    const durationMinutes = Math.round((endedAt.getTime() - workoutStarted.getTime()) / 60000);
  ...
  if (workoutEnded) {
    ...
    exercises.forEach((ex, i) => {
      ...
      for (const s of data.series) {
        if (s.completed && s.weight) totalVolume += s.weight * s.reps;
      }
    });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-zinc-950 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5 pb-8">
        {/* GIF */}
        <motion.div key={currentExercise.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }} className="aspect-square bg-black rounded-3xl overflow-hidden border-2 border-spartan-500/20 shadow-[0_0_30px_rgba(190,11,60,0.3)]">
              ...
              return (
                <div key={seriesIdx} className={`flex items-center gap-2 p-3 rounded-2xl border transition-colors ${isDone ? "bg-emerald-500/5 border-emerald-500/20" : "bg-white/[0.03] border-white/[0.06]"}`}>
                  ...
                  <input
                    ...
                    min={0} step={0.5}
                    value={entry.weight ?? ""}
                    onChange={(e) => updateSeriesField(seriesIdx, "weight", e.target.value ? parseFloat(e.target.value) : null)}
                    placeholder="kg"
                    disabled={isDone}
                    className="w-14 bg-white/5 border border-white/10 text-white text-sm font-bold text-center py-2 rounded-lg focus:outline-none focus:border-spartan-500/50 disabled:opacity-30 placeholder:text-zinc-600"
                  />
                  ...
                  <input
                    ...
                    min={1} max={99}
                    value={entry.reps}
                    onChange={(e) => updateSeriesField(seriesIdx, "reps", parseInt(e.target.value) || 1)}
                    disabled={isDone}
                    className="w-14 bg-white/5 border border-white/10 text-white text-sm font-bold text-center py-2 rounded-lg focus:outline-none focus:border-spartan-500/50 disabled:opacity-30 placeholder:text-zinc-600"
                  />
                  ...
                  <button
                    onClick={() => toggleSeriesComplete(seriesIdx)}
                    className={`ml-auto w-8 h-8 rounded-full flex items-center justify-center transition-all active:scale-[0.97] ${isDone ? "bg-emerald-500/20" : "bg-white/5 border border-white/10 hover:bg-emerald-500/20 hover:border-emerald-500/30"}`}
                  >
        ...
        {restTimer && restTimer > 0 && (
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center py-6">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs font-bold text-zinc-300">{restTarget}s</span>
            </div>
            ...
            <button
              onClick={() => { setRestTimer(null); advanceToNext(); }}
        ...
        {!restTimer && (
          <div className="grid grid-cols-2 gap-3 pt-2">
            <button onClick={skipExercise} disabled={isLastExercise && !allSeriesCompleted} className="flex items-center justify-center gap-2 px-4 py-3 rounded-2xl bg-white/5 border border-white/10 text-zinc-400 text-sm font-bold hover:bg-white/10 transition-colors active:scale-[0.97] disabled:opacity-30">
