...
export async function POST(request: Request) {
  ...
  const { module_id, title, type, order, content_json, video_url } = body;
  if (!module_id || !title) return NextResponse.json({ error: "module_id and title required" }, { status: 400 });
  const { data, error } = await supabase.from("lessons").insert({
    module_id, title, type: type || "theory", order: order || 0, content_json: content_json || {}, video_url: video_url || null
  }).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ data });
}
...
export async function PUT(request: Request) {
  ...
  for (const k of ["title", "type", "order", "content_json", "video_url"]) { if (updates[k] !== undefined) valid[k] = updates[k]; }
