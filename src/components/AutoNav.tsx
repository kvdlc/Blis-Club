    ...
    <>
      ...
      <aside className="hidden md:flex flex-col fixed left-0 top-0 bottom-0 w-60 bg-white/80 backdrop-blur-xl border-r border-zinc-100 p-6 z-40">
        <button onClick={() => goToTab("inicio")} className="flex items-center gap-2.5 px-2 py-2 mb-10">
          <div className="w-9 h-9 rounded-xl bg-auto-600 flex items-center justify-center">
            <span className="text-white font-bold text-sm">B</span>
          </div>
          <span className="text-xl font-bold text-auto-700 tracking-tight">Blis Club</span>
        </button>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium transition-all text-left w-full ${
                  active ? "bg-auto-100 text-auto-700 shadow-sm" : "text-zinc-500 hover:bg-zinc-50"
                }`}
              >
          ...
          {mobileTabs.map((tab) => {
            ...
            if (tab.center) {
              return (
                ...
                >
                  <div className="w-16 h-16 rounded-full bg-auto-600 text-white flex items-center justify-center shadow-[0_8px_24px_-4px_rgba(190,11,60,0.55)] border-4 border-white transition-transform active:scale-95">
            ...
            return (
              <button
                ...
                className={`flex items-center justify-center rounded-2xl p-3 transition-colors ${
                  active ? "text-auto-600" : "text-zinc-400"
                }`}
              >
