...
export default function AutoNav() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col fixed left-0 top-0 bottom-0 w-60 bg-zinc-950/90 backdrop-blur-xl border-r border-white/5 p-6 z-40">
        <button onClick={() => goToTab("inicio")} className="flex items-center gap-2.5 px-2 py-2 mb-10">
          ...
          <span className="text-xl font-bold text-zinc-200 tracking-tight">Blis Club</span>
        </button>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium transition-all text-left w-full ${
                  active ? "bg-auto-600/15 text-auto-400 shadow-sm border border-auto-600/20" : "text-zinc-500 hover:bg-white/5 hover:text-zinc-300"
                }`}
              >
      ...
      <nav className="md:hidden fixed bottom-4 left-4 right-4 z-40">
        <div className="bg-zinc-950/90 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_32px_-8px_rgba(0,0,0,0.6)] border border-white/8 px-3 py-2 flex items-center justify-around h-16">
          {mobileTabs.map((tab) => {
            const active = isActive(tab.key);
            if (tab.center) {
              return (
            ...
            return (
              <button
                ...
                className={`flex items-center justify-center rounded-2xl p-3 transition-colors ${
                  active ? "text-auto-400" : "text-zinc-500 hover:text-zinc-300"
                }`}
              >
