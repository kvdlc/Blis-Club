...
export function CountrySelect({ value, onChange, placeholder = "Seleccionar país" }: Props) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        {selected ? (
          <>
            ...
            <span className="flex-1 text-zinc-900 dark:text-zinc-100">{selected.name}</span>
          </>
        ) : (
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-xl max-h-60 overflow-hidden">
            ...
            {filtered.map((c) => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors ${
                  c.cca2 === value ? "bg-primary-50 dark:bg-primary-950/30" : ""
                }`}
              >
